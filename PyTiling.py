...
def Tile_Header_Cube(ra_pixel, dec_pixel, cdelt, naxis, crval_ra, crval_dec):
    ...
    HPX_2D_header += (
        "NAXIS3  =           %d / length of data axis 2 \n" % no_of_frequencies
    )
    ...
    HPX_2D_header += (
        "CRPIX2  =           %r / Coordinate reference pixel \n" % dec_pixel
    )
    ...
    HPX_2D_header += (
        "CRVAL1  =             0. / [deg] Right ascension at the reference point \n"
    )
    HPX_2D_header += (
...
if __name__ == "__main__":
    ...
    global freq_name, freq_cval, freq_cdelt, freq_cpix, freq_unit, stokes_name, stokes_cval, stokes_cpix, stokes_cdelt, no_of_frequencies, bmaj, bmin, bpa
    ...
    if no_of_frequencies == 1:
        print(
            ">>> Image provided has 1 channel, so it will be treated as an MFS, projected using mProject."
        )
    if no_of_frequencies > 1:
    ...
    for i, (ra, dec) in enumerate(zip(CRPIX_RA, CRPIX_DEC)):
        ...
        header_output_file = outdir_tile + "%s-%s-header-%d.hdr" % (
            outprefix_fitstiles,
            SB_ID,
            healpix_pixels[i],
        )
